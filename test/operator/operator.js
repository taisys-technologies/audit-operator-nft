...
const getNow = async () => {
  ...
  return now > block.timestamp ? now : block.timestamp;
};
...
describe("OperatorNFT", function () {
  ...
  let signers;
  let levels = [
    { price: ethers.utils.parseEther("1"), voter: 1, deadline: 60 * 60 },
    { price: ethers.utils.parseEther("2"), voter: 2, deadline: 60 },
    { price: ethers.utils.parseEther("3"), voter: 3, deadline: 60 * 24 * 7 },
  ];
  ...
  describe("createPoll", function () {
    ...
    it("Positive", async function () {
      ...
      const tx = await operator.connect(signer2).createPoll(1);
      await tx.wait();

      // check signer2 poll
      const poll = await operator.poll(signer2.address);
      expect(poll.poll.period).to.equal(1);
    });
    ...
    it("Negative/InValidLevel", async function () {
      ...
      const tx = operator.connect(signer2).createPoll(5);
    ...
    it("Negative/HasPollAlready", async function () {
      ...
      const tx2 = operator.connect(signer2).createPoll(1);
  ...
  describe("vote", function () {
    it("Positive", async function () {
      ...
      // signer2 create poll
      const voteLevel = 1;
      const createPoll = await operator.connect(signer2).createPoll(voteLevel);
      await createPoll.wait();

      // signer3 vote for signer2
      await VegasONE.mint(signer3.address, levels[voteLevel - 1].price);
      let approve = VegasONE.connect(signer3).approve(
        operator.address,
        levels[voteLevel - 1].price
      );
      await approve;
      ...
      expect(poll.poll.voter).to.equal(1);
    });
  ...
  describe("checkTokenAndMint", function async() {
    ...
    it("Negative/UsedUUID", async function () {
      ...
      let now = await getNow();
      let tokenDeadline = now + 7 * 24 * 60 * 60;
  ...
  describe("PollStatus", function async() {
    ...
    it("Positive/Expired/Deadline", async function () {
      ...
      // signer2 create poll
      const createPoll = await operator.connect(signer2).createPoll(2);
      await createPoll.wait();
  ...
  describe("transferAdmin", function () {
    it("Positive", async function () {
      const transferAdmin = await operator.transferAdmin(signer1.address);
      await transferAdmin.wait();

      const updateAdminTx = await operator
        .connect(signer1)
        .updateAdmin(owner.address);
      await updateAdminTx.wait();

      const tx = await operator.hasRole(ADMIN, signer1.address);

      expect(tx).to.equal(true);
    });

    it("Negative/ErrIntransition", async function () {
      const transferAdminTx = await operator.transferAdmin(signer1.address);
      await transferAdminTx.wait();

      const transferAdmin2Tx = operator.transferAdmin(signer2.address);

      await expect(transferAdmin2Tx).to.be.revertedWith("ErrIntransition");
    });

    it("Negative/ErrNotIntransition", async function () {
      const grantRoleTx = await operator.grantRole(ADMIN, signer1.address);
      await grantRoleTx.wait();

      const tx = operator.connect(signer1).updateAdmin(owner.address);

      await expect(tx).to.be.revertedWith("ErrNotIntransition");
    });

    it("Negative/ErrInvalidTransition", async function () {
      const transferAdminTx = await operator.transferAdmin(signer2.address);
      await transferAdminTx.wait();

      const grantRoleTx = await operator.grantRole(ADMIN, signer1.address);
      await grantRoleTx.wait();

      const updateAdminTx = operator
        .connect(signer1)
        .updateAdmin(owner.address);
    ...
    it("Negative/ErrGrantRoleInValidAddress/Address(0)", async function () {
      const tx = operator.transferAdmin(emptyAddress);
    ...
    it("Negative/ErrGrantRoleInValidAddress/MsgSenderAddress", async function () {
      const tx = operator.transferAdmin(owner.address);
